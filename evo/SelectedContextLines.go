...
var promptSelectContextlines = template.Must(template.New("SimplifyLLMPrompt").Parse(`
{{.LinedContext}}
...
var toolSelectContextLines = tool.NewTool("SimplifyPromptByRemoveContextWithComment", "从原始上下文中进一步对实现目标不必要的上下文，或使用注释来简化上下文，为后续更大的模型进行必要的上下文增强预处理。", func(commits *SimplifyLLMPrompt) {
