...
func (u *BusinessPlansPWA) GetId() string {
	return u.Id
}
func (u *BusinessPlansPWA) ScoreAccessor(delta ...int) int {
	...
	if u.Votes < 0 {
		keyBusinessPWA.ConcatKey("Expired").HSet(u.Id, u)
		keyBusinessPWA.HDel(u.Id)
	} else {
		keyBusinessPWA.HSet(u.Id, u)
	}
	return int(u.Votes)
}
func (u *BusinessPlansPWA) String(layer ...int) string {
	numLayer := append(layer, 0)[0]
	indence := strings.Repeat("\t", numLayer)
	childrenStr := strings.Builder{}
	for _, v := range lo.Filter(BusinessListPWA, func(v *BusinessPlansPWA, i int) bool { return v.ParentId == u.Id }) {
...
var AgentBusinessPlansPWAEd = agent.NewAgent().WithTemplate(template.Must(template.New("utilifyFunction").Parse(`
...
- Online Web Apps
- Daily Utility Apps
- LLM Generated Solutions & ELO to dispose bad   solutions & finally evolute solutions
- Deliver App via PWA
- Deliver App via Chrome Extension
...
预期这些创业项目/BusinessPlansPWA在接下来的世界中，能够产生最大化的联合的商业效用，以产生强大的社会效用。
...
`))).WithToolCallMutextRun().WithTools(tool.NewTool("SolutionRefine", "Vote on items to refine solution; Propose new solution item to parent Item if needed.", func(model *prototype.SolutionRefine) {
	...
	for ParentId, v := range model.NewProposedItems {
		...
		if len(v) > 8 {
			item := &BusinessPlansPWA{Votes: 1, Item: v, Id: utils.ID(v, 4), ParentId: ParentId}
			keyBusinessPWA.HSet(item.Id, item)
		}
	}
}))

func BusinessPlansPWAExploration() {
	// Create a new weighted chooser
	const MaxThreads = 4
	MaxThreadsSemaphore := make(chan struct{}, MaxThreads)

	for i, TotalTasks := 0, 4000; i < TotalTasks; i++ {
		MaxThreadsSemaphore <- struct{}{} // Acquire a spot in the semaphore
		go func() {
			defer func() { <-MaxThreadsSemaphore }()
			best, _ := keyBusinessPWA.HGetAll()
			BusinessListPWA = lo.Values(best)
			slices.SortFunc(BusinessListPWA, func(a, b *BusinessPlansPWA) int {
				return -(a.ScoreAccessor() - b.ScoreAccessor())
			})
			//print the lefts
			RootList := lo.Filter(BusinessListPWA, func(v *BusinessPlansPWA, i int) bool { return v.ParentId == "" })
