...
var AgentBusinessPlansEdTechEd = agent.Create(template.Must(template.New("utilifyFunction").Parse(`
...
func BusinessPlansEdTechExploration() {
	...
	for i, TotalTasks := 0, 3000; i < TotalTasks; i++ {
		...
		go func() {
			...
			err := AgentBusinessPlansEdTechEd.WithModels(models.Qwq32B).Call(param)
			if err != nil {
