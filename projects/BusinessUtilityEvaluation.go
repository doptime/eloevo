...
func (u *BusinessUtilityEvaluation) ScoreAccessor(delta ...int) int {
	var eloDelta int = append(delta, 0)[0]
	return mixincached.WithElo("projects", "BusinessUtilityEvaluation", 1000).ScoreAccessor(u.Id, float64(eloDelta))
}

var keyBusinessUtilityEvaluation = redisdb.NewHashKey[string, *BusinessUtilityEvaluation](redisdb.Opt.Rds("projects"))
...
var AgentBusinessUtilityEvaluation = agent.Create(template.Must(template.New("utilifyFunction").Parse(`
...
func BusinessUtilityEvaluationExploration() {
	...
	for i, TotalTasks := 0, 1000*1000; i < TotalTasks; i++ {
		...
		go func() {
			...
			err := AgentBusinessUtilityEvaluation.WithModels(models.Qwq32B).Call(param)
			if err != nil {
