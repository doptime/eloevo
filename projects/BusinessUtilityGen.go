...
type BusinessUtility struct {
	Id              string `description:"Id, string, unique"`
	BusinessUtility any    `description:"BusinessUtility, string, with business factors to build a business utility function"`
	Annotation      string `description:"Annotation, string, key consideration for this business utility function"`
}

func (u *BusinessUtility) GetId() string {
	return u.Id
}
func (u *BusinessUtility) ScoreAccessor(delta ...int) int {
	var eloDelta int = append(delta, 0)[0]
	return mixincached.WithElo("projects", "BusinessUtility", 1000).ScoreAccessor(u.Id, float64(eloDelta))
}

var keyBusinessUtility = redisdb.NewHashKey[string, *BusinessUtility](redisdb.Opt.Rds("projects"))
...
var AgentBusinessUtility = agent.Create(template.Must(template.New("utilifyFunction").Parse(`
...
第{{$index}}个选项：
	Id:{{$item.Id}}
	BusinessUtility:{{$item.BusinessUtility}}
...
步骤3. 在讨论的基础上，进一步提出一个更好的，新的方案草稿。对草稿同样按步骤1改进后，调用FunctionCall:BusinessUtility 保存新的方案。
{{end}}
`))).WithToolCallMutextRun().WithTools(tool.NewTool("SaveItemsSorted", "Save sorted Items, Items represented as Id list.", func(model *prototype.ItemsSorted) {
	...
	all, _ := keyBusinessUtility.HGetAll()
	sortedElos := elo.ToSlice(all).Sort().Reverse()
	elo.BatchUpdateRanking(sortedElos.TakeByIds(model.ItemsRefById...)...)

	for _, v := range lo.Filter(sortedElos, func(v elo.Elo, i int) bool { return v.ScoreAccessor() < 996 }) {
		keyBusinessUtility.ConcatKey("Expired").HSet(v.GetId(), v)
		keyBusinessUtility.HDel(v.GetId())
	}

	for i, v := range sortedElos[0:min(10, len(sortedElos))] {
		fmt.Println("Best Model,top ", i+1, v.GetId(), "Elo", v.ScoreAccessor())
	}

})).WithTools(tool.NewTool("NewBusinessUtility", "create a NewBusinessUtility plan", func(model *BusinessUtility) {
	model.Id = redisdb.NanoId(8)
	keyBusinessUtility.HSet(model.Id, model)
}))

func BusinessUtilityExploration() {
	keyBusinessUtility.HSet("default", &BusinessUtility{Id: "default", BusinessUtility: "UtilityFunction = exp(WeightMarketSizeln(MarketSize) + 0.18ln(MarketGrowthRate) + 0.22ln(ExpectedReturn) + 0.10ln(TechnicalFeasibility) + 0.15ln(InnovationPotential) + 0.080ln(ResourceAllocation) - 0.12ln(ProjectRisk + 1) - 0.080ln(CompetitionIntensity) - 0.10ln(ImplementationDifficulty) + 0.060ln(TimeToMarket) + 0.040ln(TeamExperience) + 0.050ln(PolicySupport))"})
	const MaxThreads = 16
	...
	for i, TotalTasks := 0, 1000*1000; i < TotalTasks; i++ {
		...
		go func() {
			...
			best, _ := keyBusinessUtility.HGetAll()
			//put new nodes to the top
			NodesWithPriority := lo.Filter(lo.Values(best), func(v *BusinessUtility, i int) bool {
				return v.ScoreAccessor() > 996 && v.ScoreAccessor() < 1004
			})
			selected := lo.Union(NodesWithPriority, lo.Values(best))
			mutable.Shuffle(selected[len(NodesWithPriority):])
