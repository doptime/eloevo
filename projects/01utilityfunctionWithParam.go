...
var expiredSchemaKey = redisdb.NewHashKey[string, *EvaluationSchema](redisdb.Opt.Rds("projects").Key("UtilityFunctionSchemaExpired"))
...
var evaluationSchemaAgent = agent.Create(template.Must(template.New("evaluationSchemaPrompt").Parse(`
	...
	for i := 0; i < numThreads; i++ {
		go func() {
			...
			for j := 0; j < numCallsPerThread; j++ {
				...
				err := evaluationSchemaAgent.WithModels(models.FuseO1).Call(map[string]any{
