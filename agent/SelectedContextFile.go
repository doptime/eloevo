...
`))).WithModels(models.Oss120b).WithTools(tool.NewTool("SimplifyPromptByKeepRelatedFilesOnly", "从原始context中进一步对实现目标不必要的context，或使用注释来简化context，为后续更大的模型进行必要的context增强预处理。", func(commits *SelectLLMContextFiles) {
	commits.Result = commits.RelatedFileNames
})).WithModels(models.Qwen3B30A3b2507)
